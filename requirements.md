...
| :--- | :--- | :--- | :--- | :--- |
| **`cc`** | N/A | `pp` | **The Wrapper** | The CLI tool that executes Pipe prompts. |
...
1.  **`mise.md` (Alias: `prep`)**:
    ...
    *   **Directives:** Check for git initialization, check for `AGENTS.md`, ensure `cc` is in PATH, validate directory structure.
...
2.  **Implement The Pipe (`cc`):** Write the Bash script with the Regex Shim logic. Make executable.
...
*   **Test 1 (Low-Lift):** Running `cc git "create branch foo"` outputs `git checkout -b foo` instantly without markdown formatting.
